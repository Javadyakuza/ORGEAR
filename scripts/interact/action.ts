...
async function main() {
  ...
  expect(locklift.factory.getContractArtifacts("VoteTokenRoot").abi).not.to.equal(undefined, "ABI should be available");
  expect(locklift.factory.getContractArtifacts("VoteTokenRoot").tvc).not.to.equal(undefined, "tvc should be available");
  ...
  console.log("waiting fo the proposal time to end ....");
  setTimeout(async () => {
    console.log("queuing....");
    ...
    console.log(
      " the votes : ",
      await Proposalcon.methods.getPorosposalOverview({ nowTime: locklift.testing.getCurrentTime() }).call({}),
    );
    // // suppose the proposal is succesded by now
    console.log(
      "state : ",
      (await Proposalcon.methods.getProposalState({ nowTime: locklift.testing.getCurrentTime() }).call({})).state_,
    );
    //
    ...
    await locklift.tracing.trace(
      Proposalcon.methods
        .Queue({ nowTime: 21 })
        .send({ from: WalletV3.account.address, amount: locklift.utils.toNano(1) }),
    );
