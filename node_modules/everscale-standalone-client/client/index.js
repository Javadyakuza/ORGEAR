...
const runLocal = async (ctx, req) => {
    ...
    try {
        const { output, code } = nekoton.runLocal(ctx.clock, contractState.boc, functionCall.abi, functionCall.method, functionCall.params, responsible || false);
        return { output, code };
    }
    catch (e) {
        throw invalidRequest(req, e.toString());
...
const executeLocal = async (ctx, req) => {
    ...
    try {
        ...
        const result = nekoton.executeLocal(blockchainConfig, account, message, now, (executorParams === null || executorParams === void 0 ? void 0 : executorParams.disableSignatureCheck) === true, overrideBalance != null ? overrideBalance.toString() : undefined);
